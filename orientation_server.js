...
apiRouter.post('/programs/:programId/templates/attach', ensurePerm('template.update'), async (req, res) => {
  ...
  try {
    ...
    const { template, attachResult } = await withTransaction(req, async client => {
      ...
      delete overrides.template_id;
      delete overrides.templateId;
      const linkPayload = buildLinkPayloadFromTemplate(foundTemplate, overrides, req.user?.id ?? null);
      ...
      const linkedTemplate = await programTemplateLinksDao.getTemplateForProgram({
        programId,
        templateId,
        includeDeleted: false,
        db: client,
      });
      return { template: linkedTemplate ?? foundTemplate, attachResult: attachResultInner };
    });
...
app.patch('/programs/:program_id/templates/metadata', ensurePerm('template.update'), async (req, res) => {
  ...
  try {
    ...
    for (const raw of updates) {
      ...
      const linkPatch = sanitizeLinkMetadata(raw);
      const templatePatch = {};
      if (Object.prototype.hasOwnProperty.call(raw, 'label')) {
        templatePatch.label = toNullableString(raw.label);
      }
    ...
    try {
      ...
      for (const entry of normalized) {
        let entryUpdated = 0;
        if (Object.keys(entry.linkPatch).length) {
          entry.linkPatch.updated_by = req.user?.id ?? null;
          const { updated } = await programTemplateLinksDao.updateLink({
            programId: program_id,
            templateId: entry.templateId,
            patch: entry.linkPatch,
            db: client,
          });
          if (updated) entryUpdated += 1;
        }
        if (Object.keys(entry.templatePatch).length) {
          const assignments = [];
          const params = [];
          Object.entries(entry.templatePatch).forEach(([field, value]) => {
            params.push(value);
            assignments.push(`${field} = $${params.length}`);
          });
          params.push(program_id);
          const programPlaceholder = `$${params.length}`;
          params.push(entry.templateId);
          ...
          entryUpdated += result.rowCount;
        }
        totalUpdated += entryUpdated;
      }
      await client.query('commit');
    } catch (err) {
      await client.query('rollback');
