...
export function mountLegacyOrientationServer(app: Express, pool: Pool) {
  ...
  const PUBLIC_DIR = path.join(LEGACY_ROOT, 'public');
  app.use(express.static(PUBLIC_DIR));
  const SRC_DIR = path.join(LEGACY_ROOT, 'src');
  app.use('/src', express.static(SRC_DIR));
  const SHARED_DIR = path.join(LEGACY_ROOT, 'shared');
  ...
  app.get('/rbac/users', ensureAuth, async (req, res) => {
    try {
      const roles = Array.isArray(req.roles) ? req.roles : [];
  ...
  // ==== 9) Start server ====
  return { ensurePerm, userManagesProgram, deliverMail };
}
